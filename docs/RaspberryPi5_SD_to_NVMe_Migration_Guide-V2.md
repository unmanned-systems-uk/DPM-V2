...
### Step 10: Configure Boot Order for Setup Phase

Before regenerating initramfs, we need to set the boot order to allow safe testing.
...
**What this means:**
- `1` = SD card boot (primary)
- `f` = Repeat/network boot

This tells the Pi to try SD card first, providing a safety net during configuration.
...
### Step 11: Regenerate Initramfs (MOST CRITICAL STEP)
...
**After rebooting, first verify which filesystem you're actually running from:**
...
**Scenario A** (Most likely with Ubuntu):
```
Filesystem      Size  Used Avail Use% Mounted on
/dev/nvme0n1p2  XXX   XXX   XXX   X%  /
```
✅ You're already running from NVMe root! This happens because Ubuntu recognizes the `LABEL=writable` and boots from it even though boot order prioritizes SD.

**Scenario B** (Less common):
```
Filesystem      Size  Used Avail Use% Mounted on
/dev/mmcblk0p2  XXX   XXX   XXX   X%  /
```
You're running from SD card root.

**For Scenario A (Running from NVMe root):**
...
This updates the initramfs on the NVMe system you're currently running.

**For Scenario B (Running from SD card root):**

You need to chroot into the NVMe system:
...
**Verify the initramfs was updated:**

```bash
ls -lh /boot/firmware/initrd.img
```

The timestamp should be very recent (just now).

---

### Step 12: Update Bootloader (if needed)

Check if your bootloader needs updating:
...
After reboot, verify you're still on the NVMe root:
...
### Step 13: Switch to NVMe-First Boot Order

Now that initramfs has been regenerated with NVMe support, we can configure the bootloader to try NVMe first.
...
**Find the `BOOT_ORDER=` line and change it from `0xf1` to:**
...
- `6` = NVMe boot (FIRST)
- `4` = USB boot
- `1` = SD card boot (fallback)
- `f` = Repeat/network boot

This tells the Pi to try NVMe first, then USB, then fall back to SD card if needed.

Save and exit.

**Why this phased approach matters:**
- Step 10 set `BOOT_ORDER=0xf1` (SD first) for safety during configuration
- Step 11 regenerated initramfs with NVMe drivers
- Step 13 now switches to `BOOT_ORDER=0xf416` (NVMe first)
- This prevents the kernel panic that would occur if we tried NVMe-first boot before initramfs was ready

---

### Step 14: Prepare for First Boot from NVMe
...
**No need to unmount anything** - we're already running from NVMe root with everything properly configured.

---

### Step 15: First Boot from NVMe (NVMe-First Mode)

**Now reboot to activate the new boot order:**

```bash
sudo reboot
```

🤞 **Keep the SD card inserted** for this first boot attempt as a safety net.

**What should happen:**
- Bootloader tries NVMe first (due to BOOT_ORDER=0xf416)
- Finds valid boot files on NVMe
- Loads kernel and initramfs from NVMe
- Initramfs recognizes NVMe (because we regenerated it with NVMe support)
- System mounts NVMe root and boots normally
- You see the Ubuntu login screen/desktop

**If it fails:** See troubleshooting section, but likely issue is initramfs wasn't properly regenerated.

---

### Step 16: Verify You're Running from NVMe
...
### Step 17: Test Boot Without SD Card
...
### 🎯 The Winning Strategy: Phased Boot Order Approach

**What Worked:**
1. ✅ Set `BOOT_ORDER=0xf1` (SD first) during configuration
2. ✅ Complete all setup including initramfs regeneration
3. ✅ Switch to `BOOT_ORDER=0xf416` (NVMe first) after initramfs is ready
4. ✅ Reboot and enjoy NVMe boot

**What Doesn't Work:**
1. ❌ Setting `BOOT_ORDER=0xf416` immediately → Kernel panic
2. ❌ Skipping initramfs regeneration → Boot failure
3. ❌ Using GPT partition table → Bootloader won't see NVMe
4. ❌ Wrong partition type → Bootloader skips NVMe
...
### 3. Boot Order Configuration Strategy (THE KEY TO SUCCESS)

**Problem:** Setting `BOOT_ORDER=0xf416` immediately caused kernel panic "VFS: Unable to mount root fs on unknown-block(0,0)"

**Solution:** Use the phased approach implemented in Steps 10-13 of this guide:

**Phase 1 (Step 10 - Setup):** `BOOT_ORDER=0xf1` (SD card first)
- Allows safe configuration while providing fallback
- System boots from SD but actually runs from NVMe root (due to labels)
- Enables regenerating initramfs on the NVMe system
- Provides safety net if something goes wrong

**Phase 2 (Step 11):** Regenerate initramfs with NVMe support
- MUST be done while system can access NVMe
- Includes NVMe drivers in early boot environment
- This is what prevents the kernel panic

**Phase 3 (Step 13 - After initramfs update):** `BOOT_ORDER=0xf416` (NVMe first)
- Only switch after initramfs is regenerated
- Ensures bootloader tries NVMe first
- NVMe now has all necessary drivers to boot

**Why this order matters:**
- Going straight to `0xf416` = kernel panic (no NVMe drivers in initramfs)
- Using phased approach = smooth migration with safety net
- The SD card remains as a fallback throughout the process
...
**Changelog:**
- **v2.0:** Clarified phased boot order approach (Steps 10, 13), improved initramfs step with scenario handling, added verification checkpoints
