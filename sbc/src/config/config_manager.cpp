...
void ConfigManager::load(const std::string& environment) {
    ...
    Logger::info("ConfigManager: Loading configuration from " + config_dir_);

    // Load default.json (required)
    std::string default_path = config_dir_ + "/default.json";
    if (!fs::exists(default_path)) {
        Logger::error("ConfigManager: default.json not found at " + default_path);
    ...
    if (fs::exists(env_path)) {
        loadFile(env_path);
        Logger::info("ConfigManager: Loaded environment config: " + env);
    } else {
        Logger::warning("ConfigManager: Environment config not found: " + env_path);
    }
    ...
    if (fs::exists(local_path)) {
        loadFile(local_path);
        Logger::info("ConfigManager: Loaded local.json");
    }
    ...
    if (const char* ground_ip = std::getenv("DPM_NETWORK_GROUND_IP")) {
        ...
        Logger::info("ConfigManager: Applied env var DPM_NETWORK_GROUND_IP=" + std::string(ground_ip));
    }

    std::string version = config_.contains("version") ? config_["version"].get<std::string>() : "unknown";
    Logger::info("ConfigManager: Configuration loaded (env=" + env + ", version=" + version + ")");
}

void ConfigManager::loadFile(const std::string& filename) {
    std::ifstream file(filename);
    if (!file.is_open()) {
        Logger::warning("ConfigManager: Could not open file: " + filename);
        return;
    }
    ...
    try {
        ...
        file >> file_json;
        mergeJson(file_json);
        Logger::debug("ConfigManager: Loaded file: " + filename);
    } catch (const json::parse_error& e) {
        Logger::error("ConfigManager: JSON parse error in " + filename + ": " + e.what());
        throw;
    }
}
...
json ConfigManager::getValue(const std::string& path) const {
    ...
    while (std::getline(iss, token, '.')) {
        if (!result.contains(token)) {
            Logger::error("ConfigManager: Config path not found: " + path);
...
bool ConfigManager::set(const std::string& path, const json& value) {
    try {
        setValue(path, value);
        Logger::info("ConfigManager: Set " + path + " = " + value.dump());
        return true;
    } catch (const std::exception& e) {
        Logger::error("ConfigManager: Failed to set " + path + ": " + e.what());
        return false;
    }
}
...
void ConfigManager::saveLocal() {
    ...
    try {
        ...
        if (!file.is_open()) {
            Logger::error("ConfigManager: Could not open local.json for writing");
            return;
        }

        file << config_.dump(2);  // Pretty print with 2-space indent
        Logger::info("ConfigManager: Saved configuration to local.json");
    } catch (const std::exception& e) {
        Logger::error("ConfigManager: Failed to save local.json: " + std::string(e.what()));
    }
}
...
void ConfigManager::importConfig(const json& config) {
    std::lock_guard<std::mutex> lock(mutex_);
    config_ = config;
    Logger::info("ConfigManager: Imported configuration");
}
...
bool ConfigManager::validate(const json& config) const {
    ...
    if (!config.contains("version")) {
        Logger::error("ConfigManager: Missing required field: version");
        return false;
    }

    if (!config.contains("network")) {
        Logger::error("ConfigManager: Missing required field: network");
        return false;
    }
