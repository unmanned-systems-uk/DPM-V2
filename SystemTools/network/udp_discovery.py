...
class UDPDiscoverySender:
    ...
    def __init__(self, target_host: str, target_port: int,
        ...
        logger.info(f"UDPDiscoverySender initialized: {target_host}:{target_port} every {interval_seconds}s")

    def start(self):
        """Start sending discovery heartbeats"""
        if self.running:
            logger.warning("UDPDiscoverySender already running")
            return
        ...
        try:
            ...
            logger.info(f"UDPDiscoverySender started → {self.target_host}:{self.target_port}")

        except Exception as e:
            logger.error(f"Failed to start UDPDiscoverySender: {e}")
            self.running = False
            if self.sock:
                self.sock.close()
                self.sock = None
    ...
    def stop(self):
        ...
        logger.info("Stopping UDPDiscoverySender...")
        self.running = False
        ...
        logger.info("UDPDiscoverySender stopped")
    ...
    def _discovery_worker(self):
        ...
        logger.info(f"Discovery worker started, sending: {payload_str}")

        while self.running:
            try:
                # Send discovery packet
                self.sock.sendto(payload_bytes, (self.target_host, self.target_port))
                logger.debug(f"Sent discovery packet to {self.target_host}:{self.target_port}")
    ...
    def send_immediate(self):
        ...
        if not self.sock:
            logger.warning("Cannot send immediate discovery - socket not initialized")
            return
        ...
        try:
            ...
            payload_bytes = payload_str.encode('utf-8')
            self.sock.sendto(payload_bytes, (self.target_host, self.target_port))
            logger.info(f"Sent immediate discovery packet to {self.target_host}:{self.target_port}")
        except Exception as e:
            logger.error(f"Error sending immediate discovery packet: {e}")
